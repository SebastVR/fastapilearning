...
async def process_pdf_file(file: UploadFile, save_s3: SaveS3) -> Dict[str, any]:
    contents = {
        "headers": [],
        "footers": [],
        "text": "",
    ...
    # Extracción de texto
    contents["text"] = extract_text(pdf_bytes)
    ...
    # Subir imágenes a DataLake y obtener URLs
    with fitz.open("pdf", pdf_bytes) as doc:
        for page in doc:
            ...
            for img_ref in image_list:
                ...
                # Usar la instancia SaveS3 para subir la imagen y obtener la URL
                bucket_name = "project-ppe-detection-datalake"
                image_url = save_s3.upload_file_to_minio(
                    bucket_name,
                    f"images/{file.filename}-{xref}.png",
                    image_bytes,
                    "image/png",
                )
                contents["images"].append(image_url)
